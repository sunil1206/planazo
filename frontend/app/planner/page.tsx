"use client";
/**
 * /planner — AI Wedding Planning Studio (Redesigned)
...
import { Loader2, ChevronRight, ChevronLeft, Sparkles, CheckCircle2, Star, MapPin, Phone } from "lucide-react";

// ── Constants ─────────────────────────────────────────────────────────────────
const BRAND   = "#8B1A4A";   // Couple pillar: Deep Rose
const GOLD    = "#C9952A";   // Couple pillar: Gold accent
...
export default function PlannerPage() {
  ...
  const adjustPct = (key: string, val: number) => {
  ...
  const Step3 = () => (
    <Card>
      ...
      <NavButtons canNext={parseFloat(budget) >= 50000} isLast onNext={run}
  ...
  return (
    <div className="min-h-screen" style={{ background: "#F8F5F0" }}>
      ...
      <nav className="sticky top-0 z-50"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-2 shrink-0">
            ...
            <span className="font-bold text-white text-base tracking-wide">Planazo</span>
          </Link>

          {/* Desktop links */}
          <div className="hidden md:flex items-center gap-6">
            {[["Vendors", "/vendors"], ["Weddings", "/dashboard/invites"], ["Shop", "/shop"]].map(([l, h]) => (
              <Link key={h} href={h}
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/dashboard/overview"
              className="text-xs sm:text-sm font-semibold text-white px-3 sm:px-4 py-2 rounded-xl transition-opacity hover:opacity-90"
      ...
      <div className="relative overflow-hidden py-10 sm:py-14"
        style={{ background: "linear-gradient(135deg, #0D1B2A 0%, #1B0D2E 60%, #3B0E2A 100%)" }}>
        {/* Decorative blobs */}
        <div className="absolute inset-0 opacity-20 pointer-events-none"
          style={{
            backgroundImage:
              "radial-gradient(circle at 20% 50%, #C9952A 0%, transparent 40%)," +
              "radial-gradient(circle at 80% 30%, #8B1A4A 0%, transparent 40%)",
          }} />
        <div className="relative max-w-3xl mx-auto px-4 text-center text-white">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold mb-4"
            style={{ background: `${GOLD}25`, border: `1px solid ${GOLD}40`, color: GOLD }}>
            <Sparkles size={13} /> AI-Powered Wedding Planning Studio
          </div>
          <h1 className="text-2xl sm:text-4xl font-extrabold tracking-tight mb-3">
            Plan Your Perfect Wedding
          </h1>
          <p className="text-white/60 text-sm sm:text-base max-w-xl mx-auto">
            Answer a few questions — we'll instantly match the best vendors in your city
            and build a personalised budget plan for you.
          </p>
          {step === 1 && (
            <div className="flex items-center justify-center gap-6 mt-6 text-center">
              {[["4", "Simple steps"], ["2 min", "To complete"], ["Free", "Always free"]].map(([n, l]) => (
                <div key={l}>
                  <p className="text-lg font-bold" style={{ color: GOLD }}>{n}</p>
...
function generateTimeline(eventDate: string) {
  ...
  const tasks = [
    ...
    { when: "Wedding day! 🎉",    task: eventDate ? new Date(eventDate).toLocaleDateString("en-IN",{weekday:"long",year:"numeric",month:"long",day:"numeric"}) : "", done: false },
  ];
